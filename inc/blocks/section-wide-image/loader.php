<?php
/**
 * Block Loader: section-wide-image
 * 
 * This file registers the section-wide-image block with WordPress.
 ...
 * @return void
 */
function tf_register_section_wide_image_block() {
    ...
    if (class_exists('WP_Block_Type_Registry')) {
        ...
        if (method_exists($registry, 'is_registered') && $registry->is_registered('julianboelen/section-wide-image')) {
            return;
        }
    }
    ...
    wp_register_script(
        ...
        get_template_directory_uri() . '/inc/blocks/section-wide-image/editor.js',
        array(
            'wp-blocks',
            'wp-element',
            'wp-i18n',
        ...
        filemtime(get_template_directory() . '/inc/blocks/section-wide-image/editor.js'),
        true
    );

    // Register the block type
    register_block_type(__DIR__, [
        'render_callback' => 'tf_render_section_wide_image_block',
        'editor_script'   => 'julianboelen-section-wide-image-editor',
    ]);
}

/**
 * Server-side render callback for the section-wide-image block
 * 
 * @since 1.0.0
 ...
 */
function tf_render_section_wide_image_block($attributes, $content, $block) {
...
/**
 * Enqueue additional assets for the section-wide-image block
 * 
 * @since 1.0.0
 * @return void
 */
function tf_section_wide_image_block_assets() {
    // Enqueue frontend-specific styles if needed
    if (!is_admin()) {
        wp_enqueue_style(
            'section-wide-image-frontend',
            get_template_directory_uri() . '/inc/blocks/section-wide-image/style.css',
            [],
            filemtime(get_template_directory() . '/inc/blocks/section-wide-image/style.css')
        );
    }
}

// Hook into WordPress
add_action('wp_enqueue_scripts', 'tf_section_wide_image_block_assets');
add_action('enqueue_block_editor_assets', 'tf_section_wide_image_block_assets');
 ...
 */
function tf_add_section_wide_image_block_category($categories) {
...
// add_filter('block_categories_all', 'tf_add_section_wide_image_block_category', 10, 1);
 ...
 */
function tf_filter_section_wide_image_block_attributes($attributes) {
